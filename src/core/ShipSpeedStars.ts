...
export class ShipSpeedStars {
    ...
    update(dt: number) {
        ...
        for (let i = 0; i < this.max; i++) {
            ...
            // if too far behind ship, respawn ahead (with larger threshold)
            const toStar = new THREE.Vector3().subVectors(pos, this.ship.root.position);
            const ahead = toStar.dot(forward);
            if (ahead < -60) { // Much larger threshold to match new spawn area
    ...
    private respawn(i: number, initial: boolean) {
        ...
        this.velocities[i] = 0.3 + Math.random() * 2.0; // 0.3 to 2.3 range
